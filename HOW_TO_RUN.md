...
This document walks you through the full reproduction of the OneFind project, from a fresh machine to a working demo, running tests, and producing evaluation reports. Every command is shown explicitly; if you follow it in order, the project will run.
...
```bash
git clone <this repository URL> OneFind
cd OneFind
```

Replace `<this repository URL>` with the actual URL of the project repository (e.g., `https://github.com/<your-username>/OneFind-reproduction.git`).
...
**Verify**: `pip list | grep OneFind` should show `OneFind 0.1.0 ... /path/to/OneFind`.
...
```bash
OneFind search "chlorophyll" --db demo.db --mode hybrid --precision float
```
...
```bash
OneFind sweep-alpha ./sample-data --db demo.db --alphas 0,0.5,1.0 --k 4
```
...
```
OneFind serving on http://127.0.0.1:8080 (db: data/scifact.db)
```
...
- Check that the `[model]`, `[eval]`, or `[serve]` extra was installed (whichever command you tried to run). For example, `OneFind serve` requires the `[serve]` extra.
...
The first `OneFind eval` run downloads the embedding model (~90 MB) and the BEIR datasets (~5 MB). Subsequent runs use the cached files. The first query after the model loads is ~1 second; subsequent queries are sub-150 ms p95 on CPU.
...
### "Address already in use" when starting `OneFind serve`

Another process is using port 8080. Either stop that process, or pick a different port: `OneFind serve --port 8090 --db data/scifact.db`.
...
- `src/OneFind/` — the implementation.
...
OneFind eval scifact --db data/scifact.db        # reproduce 8 configs on 5.2K docs
OneFind sweep-alpha scifact --db data/scifact.db # extension alpha sweep
OneFind serve --db data/scifact.db --port 8080  # live demo UI
pytest                                         # 67 tests